            ...
            if (enabled && m_Rope.isLoaded && isBound)            {
                ...
                if (actorConstraints != null && pinBatch != null && actorConstraints.batchCount <= solverConstraints.batchCount)
                {
                    ...
                    if (pinBatchIndex >= 0 && pinBatchIndex < rope.solverBatchOffsets[(int)Oni.ConstraintType.Pinhole].Count)
                    {
                        ...
                        for (int i = 0; i < pinBatch.activeConstraintCount; i++)
                        {
                            pinBatch.parameters[i * 5] = m_Compliance;
                            pinBatch.parameters[i * 5 + 1] = m_Friction;
                            pinBatch.parameters[i * 5 + 2] = m_MotorSpeed;
                            pinBatch.parameters[i * 5 + 3] = m_MotorForce;
                            pinBatch.parameters[i * 5 + 4] = m_ClampAtEnds ? 1 : 0;
                        }
                    }
                }
            }
            ...
            if (enabled && m_Rope.isLoaded && isBound)            {
                ...
                if (actorConstraints != null && pinBatch != null && actorConstraints.batchCount <= solverConstraints.batchCount)
                {
                    ...
                    if (pinBatchIndex >= 0 && pinBatchIndex < rope.solverBatchOffsets[(int)Oni.ConstraintType.Pinhole].Count)
                    {
                        ...
                        for (int i = 0; i < pinBatch.activeConstraintCount; i++)
                        {
                            pinBatch.edgeRanges[i*2] = firstEdge.edgeIndex;
                            pinBatch.edgeRanges[i*2+1] = lastEdge.edgeIndex;
                            pinBatch.edgeRangeMus[i*2] = firstEdge.coordinate;
                            pinBatch.edgeRangeMus[i*2+1] = lastEdge.coordinate;
                            pinBatch.particleIndices[i] = currentEdge.edgeIndex;
                            pinBatch.edgeMus[i] = currentEdge.coordinate;
                        }
